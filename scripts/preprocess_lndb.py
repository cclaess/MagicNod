...
def main(args):
    ...
    scan_paths = sorted(glob(os.path.join(args.input_dir, "data*", "*.nii.gz")))
    mask_paths = sorted(glob(os.path.join(args.input_dir, "masks", "*.nii.gz")))

    for scan_path, mask_path in zip(scan_paths, mask_paths):

        # get the patient id, study id and file name
        patient_id = scan_path.split(os.sep)[-1][:-7]
        subset_id = scan_path.split(os.sep)[-2]
        ...
        for z in range(sitk_img.GetSize()[2]):
            ...
            if sitk.GetArrayFromImage(slice_msk).sum() > 0:
                ...
                out_dir_input = Path(os.path.join(args.output_dir, subset_id, "Masked"))
                out_dir_output = Path(os.path.join(args.output_dir, subset_id, "Scan"))
                out_dir_mask = Path(os.path.join(args.output_dir, subset_id, "Mask"))
                ...
                sitk.WriteImage(slice_img_rgb_masked, os.path.join(out_dir_input, f'{patient_id}_{z}.png'))
                sitk.WriteImage(slice_img, os.path.join(out_dir_output, f'{patient_id}_{z}.png'))
                sitk.WriteImage(slice_msk, os.path.join(out_dir_mask, f'{patient_id}_{z}.png'))
